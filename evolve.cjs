 ...
 *
 * v1.4.6 修正：坑库的 apps 字段此前在「命中已知坑」分支会被写入真实 r.name
 *   （见下方累加逻辑），使随包发布/上架的 pitfalls.json 携带私有项目名，
 *   与本节承诺冲突。现统一只写 APP_PLACEHOLDER，与新建坑保持同一口径。
 */
const APP_PLACEHOLDER = '<anon>';
  ...
  try {
        ...
        if (m) matched.push({ feature: f.id, error: e, pitfall: m.id });
          ...
          if (target) {
            ...
            newOnes.push({
              ...
              apps: [APP_PLACEHOLDER], // 脱敏：不含真实项目名（坑库会随包发布/回传）
              hits: 1,
    ...
    for (const m of matched) {
      ...
      if (p) {
        ...
        // v1.4.6：绝不再把真实宿主名写进坑库（此前 push(r.name) 使公开坑库携带私有项目名）
        // 原始宿主名仍按设计留在本地 learnings.jsonl，供本机跨项目诊断。
        if (r.name && !p.apps.includes(APP_PLACEHOLDER)) p.apps.push(APP_PLACEHOLDER);
      }
    }
