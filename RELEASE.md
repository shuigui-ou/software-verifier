...
- `SKILL.md`「运行底座」段曾直接写出**本机绝对路径（含 Windows 账户名）**——既是隐私泄漏，内容本身也已过期（真实加载路径由 `PW_CORE` 决定）。改为描述 `PW_CORE` 契约，并声明**包内不硬编码任何本机绝对路径**。
- `skillhub.yaml` 的 `version` 停留在 `1.4.0`（`release.cjs:86` 本就负责同步它，历次发版漏跑）。已同步为 `1.4.6`，三处版本声明点（`SKILL.md` / `skillhub.yaml` / `skillhub_meta.json`）一致。
- 这两处**不在**上一轮审计的覆盖范围内：当时只查「私有项目名」与两个版本点；换成三面扫描（安装树 / 暂存树 / 包内 zip）后才暴露。

**仓库重建处置**：旧仓库历史自 v1.0.0 起即含实名内容，且 GitHub 上"移出分支"的提交仍可按 sha 匿名访问，故只清 HEAD 不够。处置：旧库改名为 `software-verifier-legacy` 并转私有（保留历史档案），同名公开库重建后仅推入清洗后的 71 个文件。公开面复核：未登录访问 legacy 的元数据/文件/分支全部 404，而同批公开库对照组返回 200（证明探测通道有效，404 不是误判）。
