...
function assertSafeExpr(expr) {
  ...
  for (const b of BLOCKED) {
    // v1.4.3 修复（误杀合法断言）：去掉 'i' 标志，改为**大小写敏感**匹配。
    // 依据（实测，非推测）：
    //   1) JS 标识符本身大小写敏感——`typeof fUnCtIoN` 是 undefined，`PROCESS` 同理，
    //      大小写变体**不是**绕过通路；此前的 'i' 只是"以为防绕过"，实际零安全收益。
    //   2) 而 'i' 会误杀断言语义里极常见的合法写法：
    //      `typeof el.closest === 'function'`（字符串 'function' 命中 'Function'）
    //      —— 这种断言在过去会直接"安全校验未通过"，把能验的变成验不了。
    //   3) 括号取属性的真实危险写法（如 window["eval"]）仍被大小写敏感匹配拦住，
    //      因为 ['Function'] 这类字面量文本里的大小写与黑名单一致。
