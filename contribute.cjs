...
/* ── app 名匿名化（v1.4.6）────────────────────────────────────────────────
 * 背景：坑库与回传 bundle 都可能被公开（GitHub 仓库 / 市场包 / 回流语料），
 *       真实宿主名属私有信息，不应外流。此前仅 share() 末尾有一行强制，
 *       make() 路径与 merge() 入站方向都没有同等保证——单点支撑，易漏。
 * 做法：把规则放进 sanitizePit（所有出站/入站路径的唯一收口），并放行：
 *   - 占位符：<anon> / <public> / 通用
 *   - 公开站点域名：如 element-plus.org、cn.vuejs.org（对定位问题有价值，非私有）
 *   - 其余（内部项目名、含中文的工程名等）一律归为 <anon>
 * ─────────────────────────────────────────────────────────────────────── */
const APP_PLACEHOLDERS = new Set(['<anon>', '<public>', '通用']);
const PUBLIC_HOST_RE = /^[a-z0-9][a-z0-9.-]*\.[a-z]{2,}$/i;
function anonymizeApp(a) {
  const v = String(a).trim();
  if (APP_PLACEHOLDERS.has(v)) return v;
  if (PUBLIC_HOST_RE.test(v)) return v;
  return '<anon>';
}
  ...
  if (Array.isArray(p.apps)) {
    ...
    for (const a of p.apps.slice(0, MAX_APPS)) {
      if (typeof a === 'string' && a.trim()) {
        ...
        if (t) apps.push(anonymizeApp(t)); // v1.4.6：出站/入站一律匿名化（见 anonymizeApp）
      }
    }
    out.apps = apps;
  } else out.apps = [];
...
function share(opts) {
  ...
  // （v1.4.6 起 anonymizeApp 已在 sanitizePit 内生效，本行是双保险，保留）
  ...
  const _sharedNow = pits.filter(p => p.shared).length;
  console.log('[contribute] 当前共享率 ' + _sharedNow + '/' + pits.length + '（' + Math.round(100 * _sharedNow / pits.length) + '%）；未共享多为从未运行 --share，发回即进共享池。');
...
module.exports = { make, merge, status, pendingPits, setConsent, sanitizePit, sanitizeBundle, anonymizeApp };
