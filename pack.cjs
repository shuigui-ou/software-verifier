...
const SKIP = new Set(['node_modules', '.git', 'verify_report', 'shots', '.evolution',
...
const SKIP_FILE = (p) => {
  ...
  // 补丁/备份残留绝不进市场包（v1.4.5 加固：此前 .bak_pre_* 会被打包，内含修复前代码）
  if (/(\.bak|\.orig|\.old|\.tmp)(_|$)/i.test(path.basename(p))) return true;
