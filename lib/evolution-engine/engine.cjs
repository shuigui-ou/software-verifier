 ...
 *
 * 本文件是 Host A（某本地 Node 工具）evolution.cjs 语义的**配置化收敛版**：
...
function createEngineHandle(cfg, opts = {}) {
  ...
  /** gap/plan 账本默认可接受窗口（ms）：yaml gap.ttlMs 可覆盖，默认 24h（超期未闭合 → 内核 emit 落差信号） */
  function gapTtlMs() {
    const v = Number(cfg.gap && cfg.gap.ttlMs);
    return v && v > 0 ? v : 24 * 3600 * 1000;
  }
  ...
  /** 解析内核目录（loadKernelModule 与资源客户端接线共用；失败抛 EngineError） */
  ...
  function assemble() {
    ...
    try {
      ...
      // 外挂资源层客户端：经验库/解法池查询（按 fingerprint 精准检索；soft-fail，不拖垮主链路）
      // 解析顺序：ENV EVOLUTION_RESOURCES_DIR > <rootDir>/.evolution/resources；目录存在才接线（否则 resources=null，E-only 兼容旧行为）
      let resources = null;
      try {
        const rcPath = path.join(resolveKernelRoot(), 'resource-client.cjs');
        if (fs.existsSync(rcPath)) {
          const { createLocalResourceClient } = require(rcPath);
          const resDir = process.env.EVOLUTION_RESOURCES_DIR || path.join(rootDir, '.evolution', 'resources');
          if (fs.existsSync(resDir)) {
            const expPath = path.join(resDir, 'experiences.jsonl');
            const solPath = path.join(resDir, 'solutions.jsonl');
            resources = createLocalResourceClient({
              experiencesPath: fs.existsSync(expPath) ? expPath : null,
              solutionsPath: fs.existsSync(solPath) ? solPath : null,
            });
          }
        }
      } catch (e) {
        resources = null;
        log('资源层客户端装配失败，降级为 resources=null（不影响主流程）：', e.message);
      }
  ...
  /** 把信号镜像追加到 <dataDir>/signals/signals.jsonl（宿主可读记录；与内核 analyze 链路解耦） */
  function appendSignalFile(signal) {
    try {
      ...
      if (!state.signalTextByFp.has(signal.fingerprint)) {
        state.signalTextByFp.set(signal.fingerprint, String(signal.title + ' ' + signal.detail).toLowerCase());
      }
    } catch (e) {
      log('信号镜像写入失败（不影响主流程）：', e.message);
    }
  }
  ...
  function openThread({ taskId = '', title, detail = '', ttlMs } = {}) {
    ...
    try {
      ...
      // 同步落一条 I 类信号镜像到 signals.jsonl（内核 analyze 也经 ledger.detectOverdue 产出 I）
      const sig = s.signals.classifySignal({
        type: 'I',
        kind: 'hanging',
        title: String(title || '未命名长任务'),
        detail: String(detail || ''),
      ...
      appendSignalFile(sig);
      s.audit.append('HOST_THREAD_OPEN', { entry_id: entry.id, fingerprint: sig.fingerprint, title: entry.title, taskId: entry.task_id });
  ...
  /**
   * 原语 G 类 tap：期望落差（用户要点未满足 / 显式纠错 / 对账不符）
   * 写 expectation 账本（超期未闭合 → 内核 analyze 经 ledger.detectOverdue 产出 G 信号）
   * 并同步落一条 G 类信号镜像到 signals.jsonl（与 tapE 同构）。
   * @param {object} [o] { title, detail, taskId, ttlMs } ttlMs 缺省走 gapTtlMs
   */
  function tapExpectation({ title, detail = '', taskId = '', ttlMs } = {}) {
    if (!ready()) return notReady();
    const s = subs();
    if (!s) return notReady();
    try {
      const t = String(title || '期望落差');
      const d = String(detail || '');
      const sig = s.signals.classifySignal({ type: 'G', kind: 'expectation_gap', title: t, detail: d, source: 'host', taskId: String(taskId || '') });
      const entry = s.ledger.open({
        ledger: 'expectation',
        title: t,
        detail: d,
        dueAt: new Date(Date.now() + Math.max(1, Number(ttlMs) || gapTtlMs())).toISOString(),
        taskId: String(taskId || ''),
      });
      appendSignalFile(sig);
      s.audit.append('HOST_TAP_G', { fingerprint: sig.fingerprint, ledgerId: entry.id, title: t, taskId: sig.task_id });
      return { ok: true, signalId: sig.id, fingerprint: sig.fingerprint, ledgerId: entry.id };
    } catch (e) {
      log('tapExpectation 采集失败（不影响宿主主流程）：', e.message);
      return { ok: false, reason: 'kernel_error' };
    }
  }

  /**
   * 原语 P 类 tap：计划偏离（声明步骤 vs 实际轨迹）
   * 写 plan 账本（超期未闭合 → 内核 analyze 经 ledger.detectOverdue 产出 P 信号）+ 信号镜像。
   */
  function tapPlan({ title, detail = '', taskId = '', ttlMs } = {}) {
    if (!ready()) return notReady();
    const s = subs();
    if (!s) return notReady();
    try {
      const t = String(title || '计划偏离');
      const d = String(detail || '');
      const sig = s.signals.classifySignal({ type: 'P', kind: 'plan_deviation', title: t, detail: d, source: 'host', taskId: String(taskId || '') });
      const entry = s.ledger.open({
        ledger: 'plan',
        title: t,
        detail: d,
      ...
      appendSignalFile(sig);
      s.audit.append('HOST_TAP_P', { fingerprint: sig.fingerprint, ledgerId: entry.id, title: t, taskId: sig.task_id });
      return { ok: true, signalId: sig.id, fingerprint: sig.fingerprint, ledgerId: entry.id };
    } catch (e) {
      log('tapPlan 采集失败（不影响宿主主流程）：', e.message);
      return { ok: false, reason: 'kernel_error' };
    }
  }

  /**
   * 原语 I 类 tap：悬挂未完结（open 承诺超期；默认人审，绝不自动 resume）
   * 写 thread 账本 + 信号镜像（openThread 已含同类镜像，本函数供宿主显式补报悬挂）。
   */
  function tapHanging({ title, detail = '', taskId = '', ttlMs } = {}) {
    if (!ready()) return notReady();
    const s = subs();
    if (!s) return notReady();
    try {
      const t = String(title || '悬挂任务');
      const d = String(detail || '');
      const sig = s.signals.classifySignal({ type: 'I', kind: 'hanging', title: t, detail: d, source: 'host', taskId: String(taskId || '') });
      const entry = s.ledger.open({
        ledger: 'thread',
        title: t,
        detail: d,
      ...
      appendSignalFile(sig);
      s.audit.append('HOST_TAP_I', { fingerprint: sig.fingerprint, ledgerId: entry.id, title: t, taskId: sig.task_id });
      return { ok: true, signalId: sig.id, fingerprint: sig.fingerprint, ledgerId: entry.id };
    } catch (e) {
      log('tapHanging 采集失败（不影响宿主主流程）：', e.message);
      return { ok: false, reason: 'kernel_error' };
    }
  }
  ...
  function status() {
    ...
    try {
      ...
      return {
        ...
        expectationGaps: s.ledger.listOpen('expectation').length,
        planDeviations: s.ledger.listOpen('plan').length,
